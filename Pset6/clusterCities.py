...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        for i in range(len(self.points)):
            aux.append(self.points[i].distance(other.points[i]))
        return min(aux) 
    ...
    def averageLinkageDist(self, other):
        ...
        for i in range(len(self.points)):
            meanDist += self.points[i].distance(other.points[i])
            count += 1
        meanDist /= float(count)
        return meanDist
...
def readCityData(fName, scale = False):
    ...
    dataFile = open('C:/Users/accl/Desktop/PYTHON/ProblemSet6/' + fName, 'r')
    numFeatures = 0
